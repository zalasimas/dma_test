...
/**
 * Settings RS485 mode to reception or send
 * @param tx_en - 1 to go to transmission mode
 */
void SetRSMode(bool tx_en)
{
	(tx_en) ? Chip_GPIO_SetPinState(LPC_GPIO, 0, 17, 1) : Chip_GPIO_SetPinState(LPC_GPIO, 0, 17, 0);  //TX off
}


/**
 * USART (RS485) interrupt handler
 */
...
static bool dmaTXSend(uint8_t *data, int bytes)
{
	...
	dmaTXDesc[countTXDescUsed].dest = DMA_ADDR(LPC_USART0);	/* Byte aligned */
...
static void dmaRXQueue(void)
{
	...
	for (i = 0; i < UARTRXDESC; i++) {
		...
		/* Create a descriptor for the data */
		dmaRXDesc[i].source = DMA_ADDR(LPC_USART0) + 0;	/* Byte aligned */
