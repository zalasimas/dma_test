 ...
 */
uint32_t vcom_write (char *pBuf);

/**
 * Virtual com port write routine, writes "len" number of bytes, not till \0 terminator.
 * @param pBuf - pointer to Buffer to write
 * @param len  - number of bytes to write
 * @return Number of bytes written
 */
uint32_t vcom_write_buff(char *pBuf, uint32_t len);
